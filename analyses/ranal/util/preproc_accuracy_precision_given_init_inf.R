...
preproc.accuracy.precision = function(fpath) {
  ...
  (df
    %>% filter(num_incidence > 1)
    %>% group_by(dataset, disease, init_node_idx)
    %>% mutate(min_sample_interval = min(sample_interval))
    %>% ungroup
    %>% filter(min_sample_interval <= 5)
    %>% group_by(dataset, disease, sample_interval, init_node_idx)
